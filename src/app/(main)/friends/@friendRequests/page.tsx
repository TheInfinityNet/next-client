...
import { GetFriendRequestsResponseSchema } from "@/lib/api/schemas/get-friend-requests.schema";
...
import { createGetFriendRequestsQueryOptions } from "@/hooks/queries/get-friend-requests.query";

export default function Page() {
  const { data, fetchNextPage, hasNextPage, isFetching, isLoading, isError } =
    useInfiniteQuery(createGetFriendRequestsQueryOptions({ limit: 10 }));
  ...
  const friendsSuggestion = useMemo(() => {
    return data?.pages.reduce<GetFriendRequestsResponseSchema["items"]>(
      (acc, page) => {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
      ...
      >
        <UserFriendCardLoading status="RequestReceived" />
      </div>
      {isLoading && (
        <>
          <UserFriendCardLoading status="RequestReceived" />
